...
def analyzeWrapper(req, trx_id=0):
    ...
    try:
        resp_obj = DeepFace.analyze(
            instances, actions=actions, models=facial_attribute_models)
    except:
        resp_obj = jsonify({'success': "False"})
    return resp_obj
...
def Detects_face():
    ...
    #resp_obj = DeepFace.analyze(instances, actions=actions)
    try:
       resp_obj = functions.detects_faces(req["img"], detector_backend='opencv', grayscale=False, enforce_detection=True)
    except:
       resp_obj = jsonify({'success': "False"})
...
def verifyWrapper(req, trx_id=0):
    ...
    print("Input request of ", trx_id, " has ",
          len(instances), " pairs to verify")

    #--------------------------

    if model_name == "VGG-Face":
        resp_obj = DeepFace.verify(
            instances, model_name=model_name, distance_metric=distance_metric, model=vggface_model)
    elif model_name == "Facenet":
        resp_obj = DeepFace.verify(
            instances, model_name=model_name, distance_metric=distance_metric, model=facenet_model)
    elif model_name == "OpenFace":
        resp_obj = DeepFace.verify(
            instances, model_name=model_name, distance_metric=distance_metric, model=openface_model)
    elif model_name == "DeepFace":
        resp_obj = DeepFace.verify(
            instances, model_name=model_name, distance_metric=distance_metric, model=deepface_model)
    elif model_name == "DeepID":
        resp_obj = DeepFace.verify(
            instances, model_name=model_name, distance_metric=distance_metric, model=deepid_model)
    elif model_name == "ArcFace":
        resp_obj = DeepFace.verify(
            instances, model_name=model_name, distance_metric=distance_metric, model=arcface_model)
        ...
        resp_obj = DeepFace.verify(
            instances, model_name=model_name, model=models)
    else:
        resp_obj = jsonify(
